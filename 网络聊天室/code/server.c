...
void public_chatroom_inits(int socketfd){
	...
	printf("   %sAsking for connection. chatroom_fd=%d \n",inet_ntoa(acc_addr.sin_addr),chatroom_fd);
	printf("====Gryffindor Public Room is on.====\n" );
	/*
...
void advanced_client_to_admin (send_info *send,int newfd)
{
	...
	if(flag==1)  
	{
		sprintf(ret_buf ,"Lumos");
  		sprintf(ret_buf_to_admin,"Now the \tid=%s\t has been manager.",send->id);
  		write(get_sockfd(send->id),ret_buf,strlen(ret_buf)+1);
  		write(newfd,ret_buf_to_admin,strlen(ret_buf_to_admin)+1);
	}
	else
	{
		sprintf(ret_buf ,"There is no \tid=%s\t, please check.",send->id); 
...
void drop_client_to_normal (send_info *send,int newfd) 
{
	...
	while(1)
	{
 	...
 	//DEBUG("get_sockfd(send->id)=%d\n",get_sockfd(send->id));
  	if(flag==1)  
  	{
  		sprintf(ret_buf ,"Imperio");
  		sprintf(ret_buf_to_admin,"Now the \tid=%s\t has been normal user.",send->id);
  		write(get_sockfd(send->id),ret_buf,strlen(ret_buf)+1);
  		write(newfd,ret_buf_to_admin,strlen(ret_buf_to_admin)+1);
  	}
  	else
...
void admin_kick_client (send_info *send,int newfd)
{
	...
	{//DEBUG("//////0000000///\n");
		...
		sprintf(ret_admin_buf,"This user isn't online or never exists，please check all the users are online with the '-online' to make sure his/her existence.");
	...
	if((kick_client_sockfd=get_sockfd(send->id))!=0)
     	{ 
     		memset(ret_admin_buf,0,sizeof(ret_admin_buf));
     		sprintf(ret_admin_buf,"User %s is offline.",clients[i].client_name);
...
{ 
	...
	p[2] =send->id; 
    	...
    	char ret_admin_buf[100]={0};
      	char ret_shutup_client_buf[10]="Stupefy";//向被踢的用户发送的信息
      	char id_buf[5];
      	int i=0;
      	for(;i<QUEUELEN;i++)
      	{//DEBUG(" atoi(send->id) =%d\n",atoi(send->id));
	...
	if(check_online(p)==0)//if 被禁言用户不在线则提示管理员
	{
    	...
    	if((shutup_client_sockfd=get_sockfd(send->id))!=0)
     	{ 
     		memset(ret_admin_buf,0,sizeof(ret_admin_buf));
     		sprintf(ret_admin_buf,"The %s has been banned.",clients[i].client_name);
...
void get_all_online_clients (send_info *send,int newfd)
{
	...
	char buf[BUF_SIZE]={'\0'};
	char str[BUF_SIZE]={'\0'};
	char no_client_online[40]={"o client online ,only you !"};
	for(i=0;i<QUEUELEN;i++) 
   	{
	//不包含自己
...
void private_chat (send_info *send,int newfd)
{
	...
	if((tag_id=get_sockfd(send->id))!=0)
		...
		DEBUG("This user does not exist, please check!\n");
...
{
    DEBUG("singal:%d \n",signNo);
    printf("If you wanna close the server, please enter CRTL+\\. \n");
}
//初始化连接
int system_init()
{
